...
export default function CostEstimationChallenge() {
  ...
  // editable (frontend only)
  const idRef = useRef(1);
  const makeId = () => idRef.current++;
  const [items, setItems] = useState(
    CAT_ORDER.map((cat) => ({ rid: makeId(), cost_category: cat, description: "", quantity: "", unit: "", unit_price: "" }))
  );
  const updateItem = (rid, field, value) =>
    setItems((prev) => prev.map((it) => (it.rid === rid ? { ...it, [field]: value } : it)));
  const addRow = (cat) =>
    setItems((prev) => [...prev, { rid: makeId(), cost_category: cat, description: "", quantity: "", unit: "", unit_price: "" }]);
  ...
  useEffect(() => {
    const onKey = (e) => {
      ...
      if (e.key.toLowerCase() === "r") { setZoom(1); setPosition({ x: 0, y: 0 }); }
    };
    if (isModalOpen) window.addEventListener("keydown", onKey);
    return () => window.removeEventListener("keydown", onKey);
  }, [isModalOpen]);

  const handleMouseDown = (e) => { setIsDragging(true); setStartDrag({ x: e.clientX - position.x, y: e.clientY - position.y }); };
  const handleMouseMove = (e) => { if (!isDragging) return; setPosition({ x: e.clientX - startDrag.x, y: e.clientY - startDrag.y }); };
  const handleMouseUp = () => setIsDragging(false);
  const handleWheel = (e) => { e.preventDefault(); const step = 0.1; setZoom((z) => Math.max(0.5, Math.min(3, z + (e.deltaY < 0 ? step : -step)))); };

  const grouped = CAT_ORDER.map((cat) => {
    const rows = items.filter((r) => r.cost_category === cat);
    const subtotal = rows.reduce((s, r) => (s + (parseFloat(r.quantity) || 0) * (parseFloat(r.unit_price) || 0)), 0);
  ...
  return (
    <div className="cec2-page">
      ...
      <div className="cec2-wrapper">
        ...
        {loadingChallenge && <p style={{textAlign:"center", marginBottom:12}}>Loading challenge…</p>}

        {/* TOP: details + floorplan */}
        <div className="cec3-grid-top">
          <div className="cec3-stack">
            <div className="cec3-card">
              <div className="cec3-card-header"><span className="cec3-icon">🔵</span><span>Challenge Instructions</span></div>
            ...
            <div className="cec3-card">
              <div className="cec3-card-header"><span className="cec3-icon">📄</span><span>Challenge Objectives</span></div>
              <div className="cec3-card-body">
                <p>{objectives}</p>
              </div>
            </div>
          ...
          <div className="cec3-card">
            <div className="cec3-card-header"><span>{planTitle}</span></div>
            <div className="cec3-card-body">
              ...
              <div
                ...
                onClick={() => {
                  ...
                  setIsModalOpen(true); setZoom(1); setPosition({ x: 0, y: 0 });
                }}
              >
                {floorplanImg ? (
              ...
              <div className="cec3-scale"><span>0</span><span>5</span><span>10</span><span>15</span><span>20</span></div>
            </div>
          </div>
        </div>
          ...
          <div className="cec3-card">
            ...
            <div className="cec3-card-body">
                    ...
                    <React.Fragment key={g.cat}>
                      <tr className="cat-row">
                        <td colSpan={6}>
                          ...
                          <button className="add-row-btn" onClick={() => addRow(g.cat)} type="button">+ Add material</button>
                        </td>
                      </tr>
                      ...
                      {g.rows.map((r, i) => {
                        ...
                        return (
                          <tr key={r.rid}>
                            ...
                            <td>
                              <input type="number" value={r.quantity} onChange={(e) => updateItem(r.rid, "quantity", e.target.value)} />
                            </td>
                            <td>
                              <input type="text" value={r.unit} onChange={(e) => updateItem(r.rid, "unit", e.target.value)} />
                            </td>
                            <td>
                              <input type="number" value={r.unit_price} onChange={(e) => updateItem(r.rid, "unit_price", e.target.value)} />
                            </td>
                            <td>{peso(amt)}</td>
                          </tr>
                        );
              ...
              <div className="cec2-actions">
                <button className="cec2-btn draft" type="button">Save as Draft</button>
                <button className="cec2-btn submit" type="button">Submit</button>
              </div>
            </div>
          </div>
